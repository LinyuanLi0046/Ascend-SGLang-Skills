...
- 检查文档是否更新
- 推进 `state.current_step`

## 2. 数据处理脚本

### `scripts/resolve_step1_inputs.py`
...
### `scripts/slice_trace_workspace.py`
...
### `scripts/slice_kernel_workspace.py`
...
### `scripts/slice_operator_details.py`

输出：

- `artifacts/slices/operator_details_slice.csv`

### `scripts/slice_task_time_csv.py`

输出：

- `artifacts/slices/task_time_slice.csv`

### `scripts/slice_op_summary_csv.py`

输出：

- `artifacts/slices/op_summary_slice.csv`

### `scripts/build_timeline_index.py`
...
### `scripts/check_repo_divergence.py`
...
### `scripts/build_runtime_constraints.py`
...
- 在脚本内部调用 `build_graph_seed_context_for_workspace()`，追加生成 `input/graph_seed_context.json`，供 `graph_path_analyst` 在不放宽既有门禁的前提下使用
...
### `scripts/build_graph_seed_context.py`
...
- 该脚本通常由 `build_graph_forward_context.py` 内部调用生成；`prepare_agent_dispatch.py --agent-name graph_path_analyst` 会在 dispatch 前再次校验并补齐
...
### `scripts/normalize_graph_review_result.py`
...
- 为 `artifact_promotion.graph_execution_plan_updates` 与 `graph_forward_context_updates` 自动补齐缺失的 `status`
...
### `scripts/validate_graph_candidate_graph.py`
...
### `scripts/annotate_trace_view.py`

输出：

- `output/trace_view.annotated.json`

### `scripts/render_stream_span_timeline.py`
...
- Step 4/5 会自动补齐 shared deterministic freeze 链：`stack_evidence -> graph_phase_stack_evidence -> graph_execution_plan -> graph_mapping_targets -> external_mapping_targets`
- Step 5 会在 shared freeze 之后继续补齐 `repo_divergence_report.json`、`runtime_constraints.json`、`graph_forward_context.json`、`graph_seed_context.json`、`graph_operator_spans.json`
- 回写当前激活的调度状态
