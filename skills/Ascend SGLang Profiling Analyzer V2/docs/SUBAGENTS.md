...
- `step4_bootstrap_runner` -> `references/agents/step4_bootstrap_runner.md`
- `stack_mapper` -> `references/agents/stack_mapper.md`
- `graph_bootstrap_runner` -> `references/agents/graph_bootstrap_runner.md`
...
4. 子 agent 真正返回后，先运行 `scripts/record_subagent_completion.py --agent-name <agent> --task-call-id <task_agent_id>`
...
- 各 agent 的正式 `allowed_status`、脚本白名单/黑名单、输出边界，以及 `task_required / task_receipt_required` 等调度契约，以各自手册和当次 `audit/dispatch_<agent>.json` 为准；禁止主 agent 或子 agent 自行扩展解释
