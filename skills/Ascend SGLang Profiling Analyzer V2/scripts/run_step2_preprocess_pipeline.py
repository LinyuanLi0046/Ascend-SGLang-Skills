...
LOCK_SCHEMA_VERSION = "step2_wrapper_lock_v2"
STATUS_SCHEMA_VERSION = "step2_wrapper_status_v1"
TOTAL_SCRIPTS = 2
...
def log(message: str) -> None:
    print(f"[step2-wrapper] {message}", flush=True)


def lock_path_for_workspace(workspace_dir: Path) -> Path:
    return child_run_logs_dir(workspace_dir) / "step2_wrapper.lock.json"


def status_path_for_workspace(workspace_dir: Path) -> Path:
    return workspace_dir / "audit" / "step2_in_progress.json"
...
def acquire_wrapper_lock(workspace_dir: Path) -> Path:
    ...
    if existing.get("status") == "running":
        ...
        if process_is_alive(existing_pid) and existing_pid != os.getpid():
            raise RuntimeError(
                "检测到已有活跃的 Step 2 wrapper 实例正在运行；禁止重跑。"
...
def run_script(
    ...
    *,
    ...
    metadata = run_child_script_with_logs(
        ...
        repo_root=REPO_ROOT,
        log_prefix=f"step2_{Path(script_name).stem}",
...
def main() -> int:
    ...
    log(f"Step 2 wrapper 启动，workspace={workspace_dir}")

    timeline_index_path = workspace_dir / "artifacts" / "index" / "timeline_index.json"
    try:
        timeline_index_meta = run_script(
            "build_timeline_index.py",
            workspace_dir,
            lock_path,
            stage_token="build_timeline_index",
            script_index=1,
            total_scripts=TOTAL_SCRIPTS,
        )
        ...
        update_wrapper_state(
            workspace_dir,
            lock_path,
            active_stage="build_timeline_index_done",
            completed_stage="build_timeline_index",
            stage_phase="post_check_done",
            script_index=1,
            total_scripts=TOTAL_SCRIPTS,
            current_child_script="build_timeline_index.py",
            current_child_log_path=str(timeline_index_meta["combined_log_path"]),
            last_child_meta_path=str(timeline_index_meta["metadata_path"]),
            heartbeat_count=int(timeline_index_meta.get("heartbeat_count", 0) or 0),
            output_line_count=int(timeline_index_meta.get("output_line_count", 0) or 0),
            idle_seconds=0.0,
        )

        write_outputs_meta = run_script(
            "write_preprocess_step2_outputs.py",
            workspace_dir,
            lock_path,
            stage_token="write_preprocess_outputs",
            script_index=2,
            total_scripts=TOTAL_SCRIPTS,
        )
        ensure_state_artifact(workspace_dir, "timeline_index_path", timeline_index_path)
        ...
        update_wrapper_state(
            ...
            status="passed",
            active_stage="done",
            completed_stage="write_preprocess_step2_outputs",
        ...
        log(f"Step 2 wrapper 全部完成，总耗时 {time.perf_counter() - start_ts:.2f}s")
        return 0
    except Exception:
