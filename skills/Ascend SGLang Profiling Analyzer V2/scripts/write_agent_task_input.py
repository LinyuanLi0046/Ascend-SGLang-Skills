...
def build_payload(workspace_dir: Path, agent_name: str) -> tuple[Path, dict[str, Any]]:
    ...
    if agent_name == "profiling_preprocessor":
            ...
            payload.update(
                {
                    ...
                    "must_not_do": [
                        ...
                        "不得在 wrapper 失败后临时 debug 分析文件索引延迟或 state 竞争。",
            ...
            payload.update(
                {
                    ...
                    "must_not_do": [
                        ...
                        "不得在 wrapper 失败后临时 debug 分析索引可见性或 state 时序。",
        ...
        scope_gate_path = workspace_dir / "output" / "scope_gate_result.json"
        payload.update(
            {
                "goal": "基于 timeline_index 与 classified_spans 审阅时序语义分析结果，确认 stream_role、parallel_group、粗语义分类与排除标记是否合理；Step3 不负责 graph candidate 或 phase 判定。",
                "scope_gate_result_path": str(scope_gate_path) if scope_gate_path.exists() else "",
                ...
                "acceptance_checks": [
                    ...
                    "timeline_analysis.json 至少包含 streams、parallel_groups、source",
                    "必须参考 scope_gate_result.json 中的强制排除/保留结论，不能重新放宽边界。",
                    "Step3 不得输出 graph candidate 或 phase 级正式判断。",
                    "若仍有 runtime_control span 落在 semantic 集合，必须在 timeline_analysis.json 中显式解释，不得静默略过。",
                ],
            }
        )
        ...
        payload.update(
            {
                "goal": "基于 stack / tracer 证据，在 external_mapping_targets.json 已冻结的封闭 formal target set 内完成 graph 外逐 span 正式代码定位；Step4 不负责 graph phase 判定，也不得回写 shared graph scope 工件。",
        ...
        payload.update(
            {
                "goal": "基于 shared stage 已冻结的 graph inventory / phase windows / graph_mapping_targets / graph_operator_spans，完成 graph 内真实路径重建，并生成逐 span alignment。",
                ...
                "must_not_do": [
                    ...
                    "若输出 status=partial，必须同时给出非空 blocking_issues，并继续提供可 promotion 的 graph_execution_plan_updates、graph_forward_context_updates、graph_span_candidates_payload、forward_segment_template_payload、graph_span_alignment_payload，不能只写一个轻量占位结论。",
                    "不得在最终 blocked 结论里自行猜测某个 repo 文件缺失；文件存在性结论只能引用 repo_file_existence_facts 中的正式事实，或直接以当前 repo 实际可读文件为准。",
                ...
                "acceptance_checks": [
                    ...
                    "只有当 path_reconstruction_readiness.status=ready 时，才允许继续做 graph 内真实路径重建；否则必须明确 blocked/partial 原因。",
                    ...
                    "若 status=partial，则 review_outcome 必须为 blocked，必须提供非空 blocking_issues，且仍需提交完整 artifact_promotion，保证主链能形成可审计的 graph 工件。",
                    ...
                    "若 status=partial，则 graph_span_alignment_payload 中至少要有一条记录明确体现未完成下钻状态，例如 requires_further_drilldown=true 或 location_kind 仍非 operator_call；如果所有条目都已满足最终通过条件，就不应继续停在 partial。",
                    "graph_span_alignment_payload 的 graph 条目必须显式携带 graph_operator_span_id，且该 id 必须能在 graph_operator_spans.json 中回溯到对应 operator span。",
                    "artifact_promotion.graph_span_candidates_payload、forward_segment_template_payload、graph_span_alignment_payload 必须使用 {'status': ..., 'row_count': N, 'rows': [...]} 包装；不得提交 phase-dict 或 items-only 结构。",
                    "graph_span_candidates_payload.rows[*].span_id 必须属于 graph_mapping_targets.json.rows[*].span_id；不得新增 formal graph target。",
                    "graph_span_alignment_payload.rows[*].span_id 必须属于 graph_mapping_targets.json.rows[*].span_id；不得新增 formal graph target。",
                    "graph_span_alignment_payload.rows[*].graph_operator_span_id 必须属于 graph_operator_spans.json，且其 span_id 必须与对应 operator span 一致。",
                    ...
                    "graph_span_alignment_payload 的正式逐 span 条目必须显式包含 span_id；不得再提交只有 group/template 级信息、无法被 Step6 直接消费的伪 alignment 结构。",
                ],
            }
        )
        ...
        payload.update(
            {
                ...
                "must_not_do": [
                    ...
                    "若 graph_span_alignment.json 不是 operator_call 级逐 span 结构，或缺少 graph_operator_spans.json，不得依赖 phase hint / graph hint / 全量 stack 扫描做静默补偿。",
                ],
                "required_post_checks": [
                ...
                "acceptance_checks": [
                    ...
                    "若 wrapper 在前置校验中发现 graph_operator_spans.json 缺失、graph_span_alignment 缺少 span_id / graph_operator_span_id，或 graph 条目仍非 operator_call，必须立即失败，不得继续静默 fallback。",
                ],
            }
        )
    else:
