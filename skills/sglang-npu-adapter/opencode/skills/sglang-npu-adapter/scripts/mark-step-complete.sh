...
raw_step = os.environ["STEP_NUM"]
# Support fractional steps (e.g., 6.5 for precision-rca, an optional sub-step after Step 6)
if "." in raw_step:
    step_num = float(raw_step)
    parent_step = int(step_num)  # e.g., 6.5 -> 6
    step_key = raw_step  # "6.5"
    prev_step_key = str(parent_step)  # "6" -- parent's snapshot
    next_step = parent_step + 1  # advance current_step past the parent
else:
    step_num = int(raw_step)
    step_key = str(step_num)
    prev_step_key = str(step_num - 1) if step_num >= 1 else None
...
prev_snapshot = snapshots.get(prev_step_key) if prev_step_key else None
...
snapshots[step_key] = current_snapshot
...
if prev_snapshot:
    print(f"  规划文档:            自 Step {prev_step_key} 起三份均已更新 ✓")
else:
