...
def send_request(url: str, messages: List[Dict], max_tokens: int = 50, timeout: int = 180) -> Dict[str, Any]:
    """发送推理请求"""
    import requests
...
def run_test_case(port: int, test_case: Dict, skip_content_check: bool = False) -> Dict:
    """运行单个测试用例"""
...
def run_all_tests(port: int, test_mode: str = "quick", skip_content_check: bool = False) -> Dict:
    """运行所有测试用例"""
    ...
    if skip_content_check:
        print("注意: 已启用 skip_content_check (dummy模式)，不校验输出内容")
    print(f"服务端口: {port}")
    print("-" * 40)
    
    for case in test_cases:
        print(f"运行用例 {case['case_id']}: {case['case_name']}...")
        result = run_test_case(port, case, skip_content_check)
...
def main():
    ...
    parser.add_argument("--skip-content-check", action="store_true", help="跳过输出内容校验（用于dummy权重测试）")
    ...
    results = run_all_tests(args.port, args.mode, args.skip_content_check)
