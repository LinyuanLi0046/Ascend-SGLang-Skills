...
| 精度 RCA 工程师 | 精度根因定位 | Step 6.5（precision_suspect=true 时） | `precision-rca` | `${TRAE_SKILLS_PATH}/sglang-npu-adapter/prompts/precision_rca.md` |
...
1. 运行 Triton 算子替换脚本（dry-run 先预览）：
   ```bash
   python "${TRAE_SKILLS_PATH}/sglang-npu-adapter/scripts/triton_op_replace.py" \
...
2. 运行 GSM8K 精度测试：
   ```bash
   python "${TRAE_SKILLS_PATH}/sglang-npu-adapter/scripts/gsm8k_accuracy_test.py" \
       --port 8000 \
...
```bash
bash "${TRAE_SKILLS_PATH}/sglang-npu-adapter/scripts/pre-step-check.sh" 6.5 "{WORKSPACE_DIR}"
```
...
2. 用脚本构建完整 query：
   ...
   ```
3. `Task(subagent_type="precision-rca", query=<文件 query_precision_rca.txt 的内容>, description="Precision RCA + Fix")`
