...
# 3. 调用 Task(Trae 风格,首字母大写,prompt 参数名为 `query`)
Task(
    subagent_type="<subagent-type>",   # 注意是 hyphen 形式
    query=<上一步 Read 的文件内容>,
    description="<3-5 词描述>"
)
```

> 三平台对比:Claude Code 用 `Agent(... prompt=...)`,OpenCode 用 `task(... prompt=...)`(小写),**Trae 用 `Task(... query=...)`**(大写 + `query` 参数名)。Trae 的 subagent 走全局命名空间,不需要本地 `agents/` 目录。

`<agent_name>` 与 `<subagent-type>` 映射:

| build-agent-query.sh 参数 | Task 工具 subagent_type | prompt 文件 |
|----|----|----|
...
- ❌ 跳过 `build-agent-query.sh`,手写 query 调用 Task —— 失去 P0 前置阅读注入
